...
router.post("/text", async (req, res) => {
  try {
    ...
    // console.log(
    //   "🚀 ~ file: openai.js:12 ~ router.post ~ text, activeChatId:",
    //   text,
    //   activeChatId
    // );
    ...
    await axios.post(
      ...
      {
        headers: {
          "Project-ID": process.env.CE_PROJECT_ID,
          "User-Name": process.env.CE_BOT_USER_NAME,
          "User-Secret": process.env.CE_BOT_USER_SECRET,
        },
      }
    );
...
router.post("/code", async (req, res) => {
  try {
    const { text, activeChatId } = req.body;
    // console.log(
    //   "🚀 ~ file: openai.js:62 ~ router.post ~ text, activeChatId:",
    //   text,
    //   activeChatId
    // );
...
router.post("/assist", async (req, res) => {
  try {
    const { text } = req.body;
    console.log("🚀 ~ file: openai.js:102 ~ router.post ~ text:", text)
